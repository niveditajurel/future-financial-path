      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg w-full px-0 py-0 bg-background shadow-2xl overflow-hidden rounded-2xl border-2 border-primary/20">
          <div className="relative">
            <button aria-label="Close chat" className="absolute top-3 right-3 text-muted-foreground hover:text-primary transition z-20 bg-background/80 backdrop-blur-sm rounded-full p-1" onClick={() => setOpen(false)}>
              <X className="w-5 h-5" />
            </button>
            <AiChatbot userId={userId} chatSessionId={chatSessionId} userProfile={userProfile} supabaseContext={supabaseContext} />
          </div>
        </DialogContent>
      </Dialog>
    </>;
